...
def PLDA_scoring_in_LDA_space(Fe, Ft, diagAC):
    ...
    return (
        np.dot(Fe * Lambda, Ft.T)
        + (Fe**2).dot(Gamma)[:, np.newaxis]
        + (Ft**2).dot(Gamma)
        + k
    )


def kaldi_ivector_plda_scoring_dense(
    kaldi_plda, x, target_energy=0.1, pca_dim=None
):
    ...
    x *= np.sqrt(x.shape[1] / np.dot(x**2, 1.0 / (acvar + 1.0)))[
        :, np.newaxis
    ]  # kaldi style length-norm
...
def read_xvector_timing_dict(kaldi_segments):
    ...
    return {
        s[0, 1]: (s[:, 0], s[:, 2:].astype(float))
        for s in np.split(segs, split_by_filename)
    }
...
def segment_to_frame_labels(
    starts, ends, labels, length=0, frame_rate=100.0, empty_label=None
):
    ...
    frms = np.repeat(
        np.r_[np.c_[[empty_label] * len(labels), labels].flat, empty_label],
        np.r_[
            np.c_[starts - np.r_[0, ends[:-1]], ends - starts].flat,
            max(0, min_len - ends[-1]),
        ],
    )
    return frms[:max_len]
...
def l2_norm(vec_or_matrix):
    ...
    if len(vec_or_matrix.shape) == 1:
        ...
        return (
            vec_or_matrix
            / np.linalg.norm(vec_or_matrix, axis=1, ord=2)[:, np.newaxis]
        )
    else:
        raise ValueError(
