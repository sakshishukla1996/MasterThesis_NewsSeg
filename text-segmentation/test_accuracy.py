    ...
    for dataset_path in dataset_folders:
        ...
        with tqdm(desc='Testing', total=len(dl)) as pbar:
            ...
            for i, (data, targets, paths) in enumerate(dl):
                ...
                # Saving the inputs and the outputs 
                prediction_paths.extend(paths)
                output_predictions.extend(output)
                output_labels.extend(targets)
                ...
                pointer = 0
                for p, o, t in zip(paths, output_prob, targets):
                    required = {
                        "path": p,
                        "target": t,
                        "predictions": output_prob[pointer: pointer+len(t)]
                    }
                    filename = str(p).split("/")[-1]
                    save_path = os.path.join("/data/koomri_output/", filename)
                    print 'Saved the file in %s'.format(save_path)

                    pointer+=len(t)
                    torch.save(required, save_path)
        ...
        logger.info('Windiff: {:.4}.'.format(calculated_windiff))
